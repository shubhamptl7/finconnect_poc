...
const loanUnderwritingService = {
  ...
  async evaluateApplication(applicationId) {
    ...
    const verifiedIncomeCents = Number(application.verified_monthly_income || application.monthly_income || 0);
    ...
    const kycVerif = (application.verifications || []).find(v => v.verification_type === 'IDENTITY');
    const isKycPassed = (user && user.status === 'active') || (kycVerif && (kycVerif.status === 'PASSED' || kycVerif.status === 'approved'));

    // Verify Rule 2: Plaid Financial Data
    const plaidVerif = (application.verifications || []).find(v => v.verification_type === 'FINANCIAL_ANALYSIS');
    ...
    const allRulesPassed = isKycPassed && isPlaidPassed && isIncomeSufficient && isDtiAcceptable && isAmountAcceptable;
    ...
    if (!allRulesPassed) {
      ...
      if (!isIncomeSufficient) failures.push(`Verified income (${(verifiedIncomeCents / 100).toFixed(2)} ${currency}) is below minimum requirement`);
      if (!isDtiAcceptable) failures.push(`Verified DTI of ${(dtiBps / 100).toFixed(1)}% exceeds 45.0% threshold`);
      if (!isAmountAcceptable) failures.push(`Requested amount (${(requestedAmountCents / 100).toFixed(2)} ${currency}) exceeds capacity limit of ${(eligibility.maxAmountCents / 100).toFixed(2)} ${currency}`);
